...
if __name__=="__main__":
    ...
    for f in os.listdir("data/collected/Dissertação/MotorcycleSpaceSpeed"):
        ...
        df_mrs = pd.read_csv(os.path.join("data/collected/Dissertação/MotorcycleRoadSection",f))

        df_mrs = df_mrs[df_mrs["virtual_lane_type"]=="Corredor Principal"].copy()

        # Filter 1
        df_mrs = df_mrs[df_mrs["virtual_lane_type"]=="Corredor Principal"]
        # Merge
        df_mrs = df_mrs.merge(df_mss,on=["id","frame"],how="left")
        # Calc prioritize (Y)
        df_mrs = df_mrs.groupby(["id","virtual_lane_type"]).agg({
            "frame":"count",
        ...
        df_mrs = df_mrs[df_mrs["frame_count"]>=45]
        ...
        # renames
        df_m1.columns = [col+"_m1" if not col in ["id","frame_count","vehicle_type_reference"] else col for col in df_m1.columns]
        df_m2.columns = [col+"_m2" if not col in ["id","frame_count","vehicle_type_reference"] else col for col in df_m2.columns]
        df_m3.columns = [col+"_m3" if not col in ["id","frame_count","vehicle_type_reference"] else col for col in df_m3.columns]
        ...
        df_m1.to_csv(os.path.join("data/collected/Dissertação/m1",f.replace(".json",".csv")),index=False)
        df_m2.to_csv(os.path.join("data/collected/Dissertação/m2",f.replace(".json",".csv")),index=False)
        df_m3.to_csv(os.path.join("data/collected/Dissertação/m3",f.replace(".json",".csv")),index=False)
