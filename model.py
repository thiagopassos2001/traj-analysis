...
class  YoloMicroscopicDataProcessing:
    ...
    def PostProcessing1(self):
        ...
        self.df[[self.id_column,self.frame_column,self.traffic_lane_column]] = self.df[[self.id_column,self.frame_column,self.traffic_lane_column]].astype(int)
    ...
    def VehicleBehind(
        ...
        if vehicle_df_analyzed.empty:
            return pd.DataFrame(columns=vehicle_df_analyzed.columns.tolist()+['distance_between_vehicles'])
    ...
    def FirstVehicleBehind(
        ...
        if report_null_value and behind_vehicle_group.empty:
            behind_vehicle_group[self.id_column] = [-1]
            behind_vehicle_group["distance_between_vehicles"] = [0]
    ...
    def GVCS_Type1(
        ...
        for _,row in df_four_wheel.iterrows():
            ...
            if not first_vehicle_behind.empty:
                motorcycle_between = self.MotorcycleBetweenVehicleLF(
                    ...
                    frame=row[self.frame_column],
                    max_long_dist_overlap=max_longitudinal_distance_overlap
                )
            ...
            # Se houver ids, calcula o instante de passagem na seção dessas motos
        ...
        for al in alignment:
            ...
            value["headway"] = hd
            headways.append(value)
        
        headways = pd.concat(headways,ignore_index=True)
        headways.insert(0,"start_frame",start_frame)
    ...
    def DischargeHeadwayMotorcycleAnalysis(
        ...
        # Veiculo lider no instante em que o seguidor surge
        df_analyzed[self.id_column] = df_analyzed.apply(lambda row:self.FirstVehicleAhead(
            row[self.id_column+"_follower"],
            row[self.frame_column+"_first_follower"],
            side_offset_vehicle=-0.3,
            ignore_vehicle_types_list=self.vehicle_category_list["two_wheel"],
            report_null_value=True
        )[self.id_column].values[0],axis=1)

        # Caso não tenha sido detecatado por entrarem temporalmente distantes, considerar o da posição anterior
        # Caso não faça sentido, será inutilizado na verificação
        # print(df_analyzed)
        for index,row in df_analyzed.iterrows():
            if (row[self.id_column]!=-1) or (row["position_queue_first_follower"]==1):
                df_analyzed.loc[index,self.id_column] = row[self.id_column]
            else:
                # print(row["id_follower"],row["position_queue_first_follower"]-1,row[self.traffic_lane_column+"_crossing_follower"])
                new_id = df_analyzed[(df_analyzed["position_queue_first_follower"]==row["position_queue_first_follower"]-1) & (df_analyzed[self.traffic_lane_column+"_crossing_follower"]==row[self.traffic_lane_column+"_crossing_follower"])]
                if not new_id.empty:
                    new_id = new_id[self.id_column+"_follower"].values[0]
                else:
                    new_id = np.nan
                df_analyzed.loc[index,self.id_column] = new_id
        df_analyzed = df_analyzed.dropna(subset=self.id_column)

        # Instante em que o líder cruzou a faixa
        # Merge
        df_analyzed = df_analyzed.merge(
            df_row_follower_crossing[[
                self.frame_column,
        ...
        # Renomear para destacar o seguidor
        df_analyzed = df_analyzed.rename(columns={
            self.frame_column:self.frame_column+"_crossing_leader",
            self.instant_column:self.instant_column+"_crossing_leader",
            self.traffic_lane_column:self.traffic_lane_column+"_crossing_leader",
            self.id_column:self.id_column+"_leader",
            self.vehicle_type_column:self.vehicle_type_column+"_leader",
            })

        df_analyzed[self.frame_column+"_crossing_leader"] = df_analyzed[self.frame_column+"_crossing_leader"].fillna(-404).astype(int)
        df_analyzed[self.frame_column+"_crossing_leader"] = df_analyzed.apply(lambda row:start_frame if row[self.id_column+"_leader"]==-1 else row[self.frame_column+"_crossing_leader"],axis=1)
        
        df_analyzed[self.instant_column+"_crossing_leader"] = round(df_analyzed[self.frame_column+"_crossing_leader"]/self.fps,2)

        df_analyzed[self.id_column+"_follower2"] = df_analyzed.apply(lambda row:self.FirstVehicleBehind(
        ...
        for index,row in df_analyzed.iterrows():
            ...
            a1 = True if row[self.instant_column+"_crossing_leader"]>=0 else False
            a2 = True if row[self.instant_column+"_crossing_follower"]>=0 else False
            a3 = True if row[self.instant_column+"_crossing_follower"]>row[self.instant_column+"_crossing_leader"] else False
            b = True if row[self.traffic_lane_column+"_first_follower"]==row[self.traffic_lane_column+"_crossing_follower"] else False
            ...
            if not a1:
                ...
                report = report + "@Líder não cruzou a faixa"
            if not a2:
                false_leader.append(row[self.id_column+"_follower"])
                report = report + "@Seguidor não cruzou a faixa"
            if not a3:
                report = report + "@Líder e seguidor passaram de em instantes contrários"
            if (not b) or (not c):
                false_leader.append(row[self.id_column+"_follower"])
                false_leader.append(row[self.id_column+"_leader"])
                report = report + "@Troca de faixa"
            if not d:
            ...
            if a1 and a2 and a3 and b and c and d and e and f:
                ...
                df_analyzed.loc[index,"headway"] = -1
                df_analyzed.loc[index,"valid"] = False
            df_analyzed.loc[index,"report"] = report
        ...
        for index,row in df_analyzed.iterrows():
            ...
            if valid:
                ...
                df_motorcycle_between_frame_row = pd.concat(df_motorcycle_between_frame_row+[pd.DataFrame(columns=[
                    self.id_column,self.frame_column
                ])],ignore_index=True)
                df_motorcycle_between_frame_row = df_motorcycle_between_frame_row.groupby(self.id_column).count().reset_index()
                df_motorcycle_between_frame_row["perc"] = df_motorcycle_between_frame_row[self.frame_column]/min_count
                id_motorcycle = id_motorcycle + df_motorcycle_between_frame_row[self.id_column].tolist()
    
                df_motorcycle_virtual_lane_frame_row = pd.concat(df_motorcycle_virtual_lane_frame_row+[pd.DataFrame(columns=[
                    self.id_column,self.frame_column
                ])],ignore_index=True)
                df_motorcycle_virtual_lane_frame_row = df_motorcycle_virtual_lane_frame_row.groupby(self.id_column).count().reset_index()
                df_motorcycle_virtual_lane_frame_row["perc"] = df_motorcycle_virtual_lane_frame_row[self.frame_column]/min_count
                id_motorcycle = id_motorcycle + df_motorcycle_virtual_lane_frame_row[self.id_column].tolist()
                
                idQmev = []
                idQmcv = []
                for m in pd.Series(id_motorcycle).unique().tolist():
                    perc_between = df_motorcycle_between_frame_row[df_motorcycle_between_frame_row[self.id_column]==m]["perc"]
                    perc_virtual_lane = df_motorcycle_virtual_lane_frame_row[df_motorcycle_virtual_lane_frame_row[self.id_column]==m]["perc"]
                    ...
                    print(row[self.id_column+"_leader"],row[self.id_column+"_follower"],m,"entre",round(perc_between,2),"corredor",round(perc_virtual_lane,2))
                    if (perc_virtual_lane+perc_between)>=0.5:
                        if (perc_between>=0.3) or (perc_between>=perc_virtual_lane):
                            idQmev.append(m)
                            print(f"moto {m} foi adicionada entre veículos")
                        else:
                            idQmcv.append(m)
                            print(f"moto {m} foi adicionada no corredor")
        ...
        df_analyzed["idQmev"] = df_analyzed.eval(df_analyzed["idQmev"].fillna("[]"))
        df_analyzed["idQmcv"] = df_analyzed.eval(df_analyzed["idQmcv"].fillna("[]"))
        ...
        for index,row in df_analyzed.iterrows():
            ...
            headway_array["crossing"].append(row[self.instant_column+"_crossing_leader"])
            headway_array["crossing"].append(row[self.instant_column+"_crossing_follower"])

            for id_motorcycle in row["idQmev"]:
                crossing = self.VechicleCrossingFromEndMWA(id_motorcycle)
                if not crossing.empty:
                    headway_array["id"].append(id_motorcycle)
                    headway_array["crossing"].append(crossing[self.instant_column].values[0])

            headway_array = pd.DataFrame(headway_array).sort_values("crossing")
            headway_array["headway"] = [headway_array["crossing"].iloc[0]] + headway_array["crossing"].iloc[:-1].tolist()
            ...
            df_analyzed.loc[index,"MaxHd"] = max_headway
    ...
    def EquivalenceFactor(
        ...
        df_row_follower_crossing = []
        for index,row in df_analyzed.iterrows():
            row_follower_crossing = self.VechicleCrossingSection(row[self.id_column],section_ref)
            df_row_follower_crossing.append(row_follower_crossing)
        df_row_follower_crossing = pd.concat(df_row_follower_crossing,ignore_index=True)
        ...
        def KeepValid(df):
            ...
            status = True
            for index,row in df.iterrows():
                if (row["position_queue_first_follower"]==1) or (index==0): #  
                    df.loc[index,"keep"] = row["valid"]
                    status = row["valid"]
                else:
                    df.loc[index,"keep"] = row["valid"] and status
                    status = df.loc[index,"keep"]
        ...
        df_headway = self.GVCS_Type1(
            section=section_ref,
            start_frame=start_frame,
            last_frame=df_analyzed[self.frame_column+"_crossing_follower"].max()
        ).rename(columns={self.id_column:self.id_column+"_follower"}).reset_index(drop=True)

        df_motorcycle = self.GroupVechiclesCrossingSection(
            section=section_ref,
            start_frame=start_frame,
            last_frame=last_frame,
        ...
        # print(df_analyzed[["id_follower","position_queue_crossing_follower","valid"]])
        ...
        for traffic_lane in sorted(df_analyzed[self.traffic_lane_column+"_first_follower"].unique().tolist()):
            ...
            df_headway_traffic_lane["valid_order"] = df_headway_traffic_lane["valid"]
            ...
            df_headway_traffic_lane["valid"] = df_headway_traffic_lane["valid_order"] * df_headway_traffic_lane["valid_headway"]
            df_headway_traffic_lane = KeepValid(df_headway_traffic_lane)
            # print(df_headway_traffic_lane[["id_follower","vehicle_type","position_queue_first_follower","headway","keep"]])

            last_instant_report = df_headway_traffic_lane[-df_headway_traffic_lane["keep"]]
            if last_instant_report.empty:
                last_instant_report = "Fim do ciclo"
            else:
                if not last_instant_report["valid_order"].iloc[0]:
                    last_instant_report = "Fluxo interrompido por algum evento"
                elif not last_instant_report["valid_headway"].iloc[0]:
                    last_instant_report = "Headway entre veículos foi elevado"
                else:
                    last_instant_report = "?"
            ...
            if df_analyzed_traffic_lane.empty:
                ...
                df_agg_traffic_lane.loc[0,"stop_condition"] = "Sem veículos válidos"
                ...
                for i in self.vehicle_category_list["four_wheel"]:
                    vehicle_i = df_analyzed_traffic_lane[df_analyzed_traffic_lane[self.vehicle_type_column+"_follower"]==i]
                    df_agg_traffic_lane.loc[0,"id"+i] = "["+",".join([str(j) for j in vehicle_i[self.id_column+"_follower"].tolist()])+"]"
